...
class ServicioUpdate(Conexion):
    ...
    def update_producto(self, id, desc, precio, cant, fecha_ven, id_categoria, imagen, estado):
        ...
        self.conexion.commit()
    def update_usuario(self, dni, nombre, usuario, clave, rol, acceso, n_caja, ciudad, imagen):
        sql = 'update usuarios set nombre = %s, usuario = %s, clave = %s, rol = %s, acceso = %s, n_caja = %s, ciudad = %s, imagen =  %s where dni = %s '
        self.cursor.execute(sql, (nombre, usuario, clave, rol, acceso, n_caja, ciudad, imagen, dni))
        self.conexion.commit()