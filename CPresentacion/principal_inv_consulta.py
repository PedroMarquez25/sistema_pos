...
from CPresentacion.principal_inv_editar import EditarProductoTop
    ...
    def populate_list(self):
        ...
        for item in data:            
            ...
            tk.Button(row_frame,text="\uf044", font=font_awesome, width=5, bg=color.BOTON_PRODUCTOS, bd = 0,
                      command=lambda i=item: self.edit_item(i)).pack(side='left',)
            tk.Button(row_frame, text="\uf1f8",font=font_awesome, width=5, bg=color.BOTON_PRODUCTOS, bd = 0,
                      command=lambda i=item, f=row_frame: self.delete_item(i, f)).pack(side='left', padx=10)
